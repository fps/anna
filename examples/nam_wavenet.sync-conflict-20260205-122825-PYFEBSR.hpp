...
namespace anna
{
  namespace examples
  {
    ...
    struct nam_wavenet
    {
      ...
      std::array<Eigen::Matrix<T, Channels1, Channels1>, Layers> m_linear_weights1;
      ...
      void process(Eigen::MatrixBase<InputType>  const & input, Eigen::MatrixBase<OutputType> const & output, const int n)
      {
        ...
        for (int layer = 0; layer < Layers; ++layer)
        {
          ...
          m_dilated_outputs1[layer].leftCols(n).noalias() += m_linear_weights1[layer] * m_dilated_inputs1[layer].leftCols(n);
        }
