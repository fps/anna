...
namespace anna
{
  namespace examples
  {
    ...
    struct nam_wavenet
    {
      ...
      std::array<nam_wavenet_layer<T, InputChannels, Channels2, KernelSize2, MaxBlockSize>, Layers> m_layers2;
      ...
      inline void process(Eigen::MatrixBase<InputType>  const & input, Eigen::MatrixBase<OutputType> const & output, const int n)
      {
        ...
        // block 2
        m_layers2[0].m_dilated_inputs.leftCols(n).noalias() = m_input_rechannel_weights2 * m_layers1[Layers-1].m_dilated_outputs.leftCols(n);

        for (int layer = 0; layer < Layers; ++layer)
        {
          auto & l = m_layers2[layer];
          anna::conv1d(l.m_dilated_weights, l.m_dilated_bias, 1 << layer, l.m_dilated_inputs, l.m_dilated_outputs, n, 2 * (1 << layer) + n, n);
          ...
          m_head2.leftCols(n).noalias() += l.m_dilated_outputs.leftCols(n);
        }
      }  
    };
  }
